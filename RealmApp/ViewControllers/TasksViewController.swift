...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let titleDone = indexPath.section == 0 ? "Done" : "Undone"
        let doneAction = UIContextualAction(style: .normal, title: titleDone) { [unowned self] _, _, isDone in
            ...
            StorageManager.shared.done(task, isComplete: isComplete) {
                ...
                let completIndex = IndexPath(row: completedTasks.index(of: task) ?? 0, section: 1)
                let indexReplace = indexPath.section == 0 ? completIndex : currentIndex
